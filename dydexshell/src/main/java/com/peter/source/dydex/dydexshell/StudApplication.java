...
//            }
            ...
            Log.i("peterLog", " dexPath==="+dexPath+" "+getPackageResourcePath());
...
//            Reflect.on(wr.get()).set("mClassLoader", dLoader);
            DyDexClassLoader dLoader = new DyDexClassLoader(this,getdexArray(dexPath),getPackageResourcePath(),odexPath,reallibpath,ClassLoader.getSystemClassLoader());
            Reflect.on(wr.get()).set("mClassLoader", dLoader);
    ...
    private byte[] getdexArray(String path){
        ByteArrayOutputStream dexByteArrayOutputStream = null;
        try {
            FileInputStream fileOutputStream =  new FileInputStream(new File(path));
            dexByteArrayOutputStream = new ByteArrayOutputStream();
            byte[] arrayOfByte = new byte[1024];
            while (true) {
                int i = fileOutputStream.read(arrayOfByte);
                if (i == -1)
                    break;
        ...
        byte[] result =  dexByteArrayOutputStream.toByteArray();
        Log.e("peterLog","////////////dexSize="+result.length);
        return result;
    }
